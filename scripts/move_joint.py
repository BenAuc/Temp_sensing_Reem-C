#!/usr/bin/python

from __future__ import print_function
import argparse
import sys
import signal
from time import time, sleep
from uuid import uuid4
import rospy
from actionlib import SimpleActionClient, GoalStatus
from play_motion_msgs.msg import PlayMotionAction, PlayMotionGoal, PlayMotionResult

# pretty - A miniature library that provides a Python print and stdout
# wrapper that makes colored terminal text easier to use (eg. without
# having to mess around with ANSI escape sequences). This code is public
# domain - there is no license except that you must leave this header.
#
# Copyright (C) 2008 Brian Nez <thedude at bri1 dot com>
#
# With modifications
# (C) 2013 Paul M <pmathieu@willowgarage.com>
# (C) 2014 Adolfo Rodriguez Tsouroukdissian <adolfo.rodriguez@pal-robotics.com>
# (C) 2022 Benoit Auclair <benoitauclair30@gmail.com

class MoveJointException(Exception):
    pass

class MotionData:
    def __init__(self, name, pos, dur):
        print("name :", name)
        print("pos :", pos)
        print("dur :", dur)
        self.motion_name = name + '_' + uuid4().hex 
        self.joint_name  = name
        self.position    = pos
        self.duration    = dur
        #self.client = None



    def wait_for_clock(self):
        timeout = time() + 2.0 # Wall-clock time
        while not rospy.is_shutdown() and rospy.Time.now().is_zero():
            if time() > timeout:
                raise MoveJointException('Timed out waiting for a valid time. Is the /clock topic being published?.')
            sleep(0.1) # Wall-clock sleep

    def play_motion_client(self, action_ns):
        #self.client = SimpleActionClient(action_ns, PlayMotionAction)
        client = SimpleActionClient(action_ns, PlayMotionAction)
        if not client.wait_for_server(rospy.Duration(1)):
            raise MoveJointException("No play_motion server running (namespace '{}')".format(action_ns))
        return client

    def load_motion(self, motion_namespace):
        motion_param = motion_namespace + '/' + self.motion_name
        rospy.set_param(motion_param + '/joints', [self.joint_name])
        rospy.set_param(motion_param + '/points', [{'positions': [self.position],
                                                    'time_from_start': self.duration}])

    def unload_motion(self, motion_namespace, motion_name):
        motion_param = motion_namespace + '/' + motion_name
        if rospy.has_param(motion_param):
            rospy.delete_param(motion_param)

    def active_cb(self):
        msg = 'Moving joint ' + bold(self.joint_name) + ' to position ' + bold(str(self.position))
        if self.duration > 0.0:
            msg += ' in ' + bold(str(self.duration)) + 's'
        print(msg)

    def move_joint_cmd(self):

        play_motion_ns = 'play_motion' # TODO: How to resolve names?
        motion_ns      = play_motion_ns + '/motions'

        try:
            print("move_joint_cmd()")
            self.wait_for_clock()
            print("move_joint_cmd()")
            client = self.play_motion_client(play_motion_ns)
            print("move_joint_cmd()")
        except MoveJointException, e:
            self.print_err(str(e))

        def cancel(signum, frame):
            client.cancel_goal()
            rospy.signal_shutdown("goal canceled")
        signal.signal(signal.SIGINT, cancel)

        self.load_motion(motion_ns)
        goal = PlayMotionGoal(motion_name = self.motion_name, skip_planning = True)
        client.send_goal(goal, None, self.active_cb)
        print("wait_for_result()")
        client.wait_for_result()
        print("wait_for_result()")

        self.unload_motion(motion_ns, self.motion_name)
        al_res = client.get_state()
        pm_res = client.get_result()
        if al_res != GoalStatus.SUCCEEDED or pm_res.error_code != PlayMotionResult.SUCCEEDED:
            self.print_err("Execution failed with status {}. {}".format(GoalStatus.to_string(al_res), pm_res.error_string))
            sys.exit(1)

    def printc(self, text, color, file = sys.stdout):
        """Print in color."""
        
        codeCodes = {
        'black':    '0;30', 'bright gray':  '0;37',
        'blue':     '0;34', 'white':        '1;37',
        'green':    '0;32', 'bright blue':  '1;34',
        'cyan':     '0;36', 'bright green': '1;32',
        'red':      '0;31', 'bright cyan':  '1;36',
        'purple':   '0;35', 'bright red':   '1;31',
        'yellow':   '0;33', 'bright purple':'1;35',
        'dark gray':'1;30', 'bright yellow':'1;33',
        'normal':   '0'
        }

        if ((file == sys.stdout and sys.stdout.isatty()) or
            (file == sys.stderr and sys.stderr.isatty())):
            print("\033["+codeCodes[color]+"m"+text+"\033[0m", file = file)
        else:
            print(text)

    def bold(self, msg):      return "\033[0;1m"+msg+"\033[0;22m"
    def print_err(self, msg): self.printc(msg, 'red', file = sys.stderr)
    def print_ok(self, msg):  self.printc(msg, 'green')
